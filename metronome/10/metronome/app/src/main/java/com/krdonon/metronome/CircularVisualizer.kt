...
fun CircularVisualizer(
    ...
    Canvas(
        ...
        // 세그먼트(눈금)가 캔버스 하단을 넘어 잘리지 않도록 여유를 조금 더 둡니다.
        ...
        for (i in 0 until beatsPerMeasure) {
            ...
            // 기존에는 강박/약박 활성 시 radius 밖으로 조금 튀어나오도록(+10f) 그려서
            // 하단에서 UI와 겹치거나 잘리는 현상이 있었습니다. 캔버스 내부에 머물도록 고정합니다.
            val segmentRadius = radius

            rotate(angle, pivot = Offset(centerX, centerY)) {
                val startY = centerY - segmentRadius
                val endY = startY + segmentWidth * 2.6f
