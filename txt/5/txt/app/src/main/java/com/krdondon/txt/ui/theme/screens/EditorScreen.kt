...
fun EditorScreen(
    ...
    // ✅ A4 페이지 가이드 보정(줄 단위). (+)면 한 페이지에 더 많은 줄이 들어가는 것으로 계산
    // 동우님 케이스가 “약 4줄 차이”라서 기본값을 4로 둡니다. 필요하면 상단 Adj +/-로 조절하세요.
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    ...
                    // ✅ A4 켰을 때만 보정 조절 버튼 표시
                    if (a4Guide) {
                        TextButton(
                            onClick = { guideAdjustLines = (guideAdjustLines - 1).coerceIn(-20, 20) }
                        ) { Text("Adj-") }

                        Text(
                            text = if (guideAdjustLines >= 0) "+$guideAdjustLines" else "$guideAdjustLines",
                            style = MaterialTheme.typography.bodySmall
                        )
            ...
            Box(modifier = Modifier.weight(1f).fillMaxWidth()) {
                ...
                if (a4Guide) {
                    ...
                    Canvas(modifier = Modifier.matchParentSize()) {
                        ...
                        // 2페이지 시작 = (effectiveLinesPerPage)줄, 3페이지 시작 = 2*effectiveLinesPerPage 줄...
