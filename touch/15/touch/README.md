...
현재 `drawable/`에는 앱 아이콘 관련 XML만 남겨 둡니다.

- `ic_launcher_background.xml`
- `ic_launcher_foreground.xml`

게임용 PNG 이미지는 이 폴더에서 제거했습니다.

### 6.2 `drawable-nodpi/`

2026-09-01 메모리 최적화 작업에서 게임용 PNG 6개를 `drawable/`에서 `drawable-nodpi/`로 이동했습니다.
...
이 이미지들은 Compose에서 `dp` 크기로 직접 표시하고 있으므로 `drawable-nodpi`에 두어 Android가 화면 밀도에 따라 원본 PNG를 불필요하게 확대 디코딩하지 않도록 했습니다. 고밀도 기기에서 비트맵 메모리 사용량을 줄이는 목적입니다.

Kotlin 코드의 리소스 참조는 그대로 `R.drawable.touch`, `R.drawable.touchimage1` 등의 형태를 사용합니다. 리소스 폴더가 `drawable-nodpi`로 바뀌어도 `R.drawable.*` 접근 방식은 바뀌지 않습니다.
...
### 생명주기 처리
- 앱이 백그라운드로 내려갈 때 BGM `MediaPlayer`의 native 리소스를 `release()`
- 앱이 다시 활성화되면 필요할 때 `MediaPlayer`를 재생성하고 BGM 상태에 따라 재생
- `applicationContext`를 사용하여 오디오 객체가 Activity Context를 불필요하게 오래 참조하지 않도록 정리
- 화면 종료 시 남아 있는 오디오 리소스도 release
...
- 현재 경과 시간 고정
- 자동 일시정지 처리
- BGM 재생을 멈추고 `MediaPlayer`의 native 리소스를 해제하여 백그라운드 메모리 점유를 줄임

### ON_STOP
- 누적 시간 저장

### ON_RESUME / 포그라운드 복귀
- 필요한 경우 BGM `MediaPlayer`를 다시 생성
- BGM 스위치가 켜져 있으면 재생 복원
- 자동 일시정지였던 경우 게임 재개

이 구조는 게임 상태를 유지하면서도 백그라운드에 오래 있을 때 BGM용 native 리소스를 계속 잡고 있지 않도록 한 메모리 최적화입니다.
...
## 15. 2026-09-01 앱 품질 / 메모리 최적화 변경 이력

2026-09-01에 Google Android Developers의 앱 품질·메모리 최적화 관련 안내를 참고하여, 이 프로젝트의 규모와 게임 특성에 맞는 항목만 선별해서 적용했습니다. 게임 로직 자체나 화면 구조를 크게 바꾸는 과도한 리팩터링은 하지 않았습니다.

### 15.1 변경된 Kotlin 파일

- **`MainActivity.kt` 변경**
  - 백그라운드 진입 시 BGM `MediaPlayer`를 단순 일시정지 상태로 계속 보유하지 않고 `release()`하도록 개선
  - 포그라운드 복귀 시 필요한 경우 BGM 플레이어를 다시 생성하도록 개선
  - 오디오 생성 시 `applicationContext`를 사용하여 Activity Context가 오디오 객체에 불필요하게 묶이지 않도록 정리
  - 기존 게임 진행시간, 난이도, 멀티터치, 타겟 생성 및 태블릿 동작은 유지

- **`TabletLandscapeScreen.kt`는 변경하지 않음**
  - 태블릿 회전 레이아웃과 기존 게임 동작은 그대로 유지

### 15.2 이미지 리소스 위치 변경

기존에는 게임 PNG 6개가 `app/src/main/res/drawable/`에 있었습니다. 현재는 아래와 같이 `drawable-nodpi/`로 이동했습니다.
...
따라서 **`drawable/`에 위 PNG 6개가 중복으로 남아 있으면 삭제해야 합니다.** `drawable-nodpi/`에 있는 파일만 유지합니다. `ic_launcher_background.xml`, `ic_launcher_foreground.xml`은 `drawable/`에 그대로 둡니다.

이 변경의 목적은 고밀도 화면에서 Android가 게임 PNG를 DPI 기준으로 불필요하게 확대 디코딩하여 메모리를 더 사용하는 것을 줄이는 것입니다.

### 15.3 R8 / 리소스 축소 설정 변경

`gradle.properties`의 R8 관련 설정을 다음과 같이 최신 최적화 방향에 맞게 조정했습니다.
...
프로젝트의 release 빌드는 기존에 이미 minify/resource shrinking을 사용하고 있으므로, 별도의 대규모 Gradle 구조 변경은 하지 않고 필요한 최적화만 활성화했습니다.

### 15.4 적용하지 않은 항목

- 로그인/계정 기능이 없는 단순 게임이므로 Zero-Tap Sign-In 같은 인증 관련 기능은 추가하지 않음
- ViewModel, Repository, UseCase 등 대규모 아키텍처 변경은 이번 작업에서 하지 않음
- 게임 난이도, 터치 판정, 멀티터치, 태블릿 회전 로직은 변경하지 않음

### 15.5 Android Studio에서 변경된 실제 폴더를 확인하는 방법

Android Studio의 왼쪽 **Project** 창은 기본적으로 `Android` 보기로 되어 있을 수 있습니다. `Android` 보기에서는 리소스가 Android 방식으로 묶여 보이기 때문에 실제 디스크 폴더 구조를 확인하기가 헷갈릴 수 있습니다.

이번 변경처럼 `drawable`과 `drawable-nodpi`가 실제로 어떻게 나뉘어 있는지 확인할 때는 **Project 창 상단의 보기 방식을 `Android`가 아니라 `Project`로 변경해서 확인하는 것이 가장 확실합니다.**
...
여기서 최종적으로 다음 상태인지 확인합니다.

- `drawable/` → `ic_launcher_background.xml`, `ic_launcher_foreground.xml`만 유지
- `drawable-nodpi/` → `touch.png`, `touchimage1.png` ~ `touchimage5.png` 유지
- 같은 PNG 파일이 두 폴더에 동시에 존재하지 않도록 함

### 15.6 나중에 다시 확인할 때 핵심 체크 항목

1. `MainActivity.kt` : BGM MediaPlayer의 백그라운드 release / 복귀 시 재생성 로직 확인
2. `res/drawable-nodpi/` : 게임 PNG 6개가 이 폴더에만 있는지 확인
3. `gradle.properties` : R8 관련 두 설정이 `true`인지 확인
4. Android Studio에서는 실제 폴더 구조 확인 시 **Project 보기**로 전환
5. release AAB를 새로 빌드하여 Play Console에 업로드할 때 최적화 결과를 다시 확인
