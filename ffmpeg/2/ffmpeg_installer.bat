...
echo [%time%] Current user PATH retrieved >> "%LOG%"

:: Check if already in user PATH
set "PATH_EXISTS=0"
if defined CURRENT_USER_PATH (
    echo [%time%] Checking if PATH contains ffmpeg >> "%LOG%"
    echo !CURRENT_USER_PATH! | find /i "%ProgramFiles%\ffmpeg\bin" >nul 2>&1
    if not errorlevel 1 (
        set "PATH_EXISTS=1"
        echo [%time%] PATH_EXISTS set to 1 >> "%LOG%"
    )
)

echo [%time%] PATH_EXISTS=%PATH_EXISTS% >> "%LOG%"

if "%PATH_EXISTS%"=="1" call :PATH_ALREADY_EXISTS
if "%PATH_EXISTS%"=="0" call :ADD_TO_PATH

echo [%time%] PATH section completed >> "%LOG%"
goto INSTALLATION_COMPLETE

:PATH_ALREADY_EXISTS
echo [%time%] PATH already contains ffmpeg\bin >> "%LOG%"
echo.
echo FFmpeg path is already in PATH.
echo Skipping PATH registration.
echo.
echo [%time%] Skipping to step 6 >> "%LOG%"
exit /b
...
if not defined CURRENT_USER_PATH (
    echo [%time%] Creating new user PATH >> "%LOG%"
    setx PATH "%ProgramFiles%\ffmpeg\bin" >nul 2>&1
) else (
    echo [%time%] Appending to existing PATH >> "%LOG%"
    setx PATH "!CURRENT_USER_PATH!;%ProgramFiles%\ffmpeg\bin" >nul 2>&1
)

:: Wait and verify
echo [%time%] Waiting for registry update >> "%LOG%"
timeout /t 2 /nobreak >nul 2>&1

:: Verify by reading registry again
set "NEW_USER_PATH="
for /f "skip=2 tokens=2*" %%a in ('reg query "HKCU\Environment" /v PATH 2^>nul') do set "NEW_USER_PATH=%%b"

echo [%time%] Verifying PATH update >> "%LOG%"
set "VERIFY_OK=0"
if defined NEW_USER_PATH (
    echo !NEW_USER_PATH! | find /i "ffmpeg\bin" >nul 2>&1
    if not errorlevel 1 (
        set "VERIFY_OK=1"
        echo [%time%] VERIFY_OK set to 1 >> "%LOG%"
    )
)

echo [%time%] VERIFY_OK=%VERIFY_OK% >> "%LOG%"

if "%VERIFY_OK%"=="1" (
    echo [%time%] PATH updated successfully verified >> "%LOG%"
    echo Done!
    echo [%time%] Exiting ADD_TO_PATH subroutine >> "%LOG%"
    exit /b
)

echo [%time%] WARNING: Could not verify PATH update >> "%LOG%"
echo.
echo ============================================
echo   WARNING: PATH update issue
echo ============================================
echo.
echo FFmpeg was installed successfully, but
echo automatic PATH registration may have failed.
echo.
echo Please add this path manually if needed:
echo %ProgramFiles%\ffmpeg\bin
echo.
echo How to add manually:
echo 1. Search "Environment Variables" in Windows
echo 2. Click "Environment Variables" button
echo 3. Under "User variables", find "Path"
echo 4. Click "Edit" and add the path above
echo.
...
echo [%time%] Reached INSTALLATION_COMPLETE label >> "%LOG%"
