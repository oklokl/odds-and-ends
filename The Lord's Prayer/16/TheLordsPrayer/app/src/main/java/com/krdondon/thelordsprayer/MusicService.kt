...
class MusicService : Service(), AudioManager.OnAudioFocusChangeListener {
    ...
    /**
     * 일시정지 시 MediaPlayer의 native/decoder 메모리를 계속 붙잡지 않도록
     * 플레이어는 release하고 재생 위치만 보관합니다.
     */
    ...
    private fun pausePlayback() {
        ...
        // Paused 상태에서 MediaPlayer/decoder native 메모리를 계속 보유하지 않습니다.
        releaseMediaPlayer(preservePosition = true)
    ...
    private fun stopPlayback() {
        ...
        resumePositionMs = 0
        releaseMediaPlayer(preservePosition = false)
    ...
    private fun selectVoiceSet(requestedSetName: String?) {
        ...
        // 빈 값/기본 선택이면 기본 음성(res/raw)을 사용
    ...
    private fun releaseMediaPlayer(preservePosition: Boolean) {
        val player = mediaPlayer ?: return
    ...
    override fun onTrimMemory(level: Int) {
        ...
        // 재생 중인 오디오는 사용자 체감 서비스이므로 유지합니다.
        // 재생 중이 아니라면 저메모리 신호에서 decoder/native 자원과 작은 캐시를 즉시 비웁니다.
