...
기본 음성(`res/raw/prayer_0815.mp3`)을 제공하며, `assets/sounds/set0`, `set1` … 폴더에 mp3를 추가하면 앱이 자동으로 인식하여 음성 세트를 전환/선택할 수 있습니다.
...
- **세트 선택(set 버튼)** 으로 특정 세트(set3 등)를 직접 선택
  - `assets/sounds/` 아래의 `set숫자` 폴더를 스캔하여 목록을 자동 생성
  - `set4`, `set5`… 폴더를 추가하면 선택 목록이 자동으로 증가(하드코딩 없음)
...
- 앱 실행 후 **“음성 변경”** 버튼을 누를 때마다 다음 세트로 넘어갑니다. (예: set0 → set1 → set2 → …)
- 선택된 세트 내 mp3 중 **1개가 랜덤 재생**됩니다.
- 재생 중에 음성을 변경하면, 변경된 세트 음성으로 **즉시** 전환됩니다.

### B) “set 버튼”(직접 선택)

- 화면에 표시되는 `set3` 같은 버튼을 누르면, 현재 앱이 인식한 `set0..setN` 목록이 표시됩니다.
- 사용자가 특정 세트를 탭하면 해당 세트가 선택되며,
  - 재생 중이면 선택한 세트로 **즉시 전환 재생**
  - 재생 중이 아니면 선택만 변경되고, 다음 재생부터 적용됩니다.
...
- `assets/sounds/set0` 같은 세트 폴더가 **아예 없거나**, 세트 내부에 mp3가 없을 때 자동으로 기본 음성이 재생됩니다.
...
### “음성 변경”/“set 선택”을 눌렀는데 mp3가 재생되지 않음
