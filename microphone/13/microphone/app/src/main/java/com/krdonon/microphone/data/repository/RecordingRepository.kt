...
class RecordingRepository(private val context: Context) {
    ...
    /**
     * tempFile 하나를 실제 녹음 폴더로 옮기고 RecordingFile 생성
     */
    ...
    suspend fun restoreFromTrash(recording: RecordingFile) = withContext(Dispatchers.IO) {
        try {
            ...
            // 2) 원래 녹음 폴더 (새 녹음이 저장되는 폴더와 동일하게)
            val recordingsDir = getRecordingsDirectory()
            if (!recordingsDir.exists()) recordingsDir.mkdirs()

            // 3) 같은 이름이 있으면 _1, _2 … 붙여서 중복 방지
            val originalName = trashFile.nameWithoutExtension
            val ext = trashFile.extension
            var restoredFile = File(recordingsDir, trashFile.name)
            var counter = 1
            while (restoredFile.exists()) {
                restoredFile = File(recordingsDir, "${originalName}_$counter.$ext")
                counter++
            }

            // 4) 휴지통 → 녹음 폴더로 복사 후 휴지통 파일 삭제
            trashFile.copyTo(restoredFile, overwrite = false)
            val deletedTrash = trashFile.delete()
            android.util.Log.d(
                "RecordingRepository",
                "Restore from trash: copy OK, trash deleted = $deletedTrash, restoredPath=${restoredFile.absolutePath}"
            )

            // 5) Flow 업데이트: trash 리스트에서 제거, 메인 리스트에 추가
            val trashList = _trashFlow.value.toMutableList()
            trashList.removeAll { it.id == recording.id }
            _trashFlow.value = trashList.sortedByDescending { it.dateCreated }
