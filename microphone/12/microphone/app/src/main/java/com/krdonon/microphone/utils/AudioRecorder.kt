    ...
    fun startRecording(outputFile: File): Boolean {
        return try {
            ...
            mediaRecorder = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                ...
                // 마이크 소스 / 포맷 / 채널 / 비트레이트 설정 기존 코드...

                // ⚠️ 파일 시스템 4GB 제한 대비: 3.7GB에서 자동 종료 이벤트
                try {
                    setMaxFileSize(3_700_000_000L)
                } catch (e: Exception) {
                ...
                setOnInfoListener { mr, what, _ ->
                    if (what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_FILESIZE_REACHED) {
                        Log.w("AudioRecorder", "Max file size reached")
                        try {
                            mr.stop()
                        } catch (e: Exception) {
                        ...
                        _recordingState.value =
                            RecordingState.Error("파일 시스템 제한 때문에 녹음이 자동으로 중지되었습니다.")
                    }
                }
    ...
    fun pauseRecording() {
        if (_recordingState.value is RecordingState.Recording) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                try {
                    mediaRecorder?.pause()
                    // 파일 시스템에 데이터를 flush하도록 강제
                    Thread.sleep(100) // 짧은 딜레이 추가
                    isPaused = true
