...
class RecordingService : Service() {
    ...
    private fun startRecording(outputFile: File) {
        serviceScope.launch {
            ...
            fileSizeJob?.cancel()
            fileSizeJob = launch {
                val maxFileSize = 3_700_000_000L // 약 3.7GB
                while (isActive) {
                    ...
                    if (size >= maxFileSize) {
                        Log.w("RecordingService", "cache file reached $size bytes, stopping recording safely")
                        // 메인 스레드에서 stop 호출
                        withContext(Dispatchers.Main) {
                            // 알림에서 정지한 것은 아니므로 false
                            stopRecording(fromNotification = false)
                        }
                        break
                    }
