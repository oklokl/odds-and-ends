...
class RecordingRepository(private val context: Context) {
    ...
    suspend fun deleteRecording(recording: RecordingFile, moveToTrash: Boolean = false) = withContext(Dispatchers.IO) {
        try {
            ...
            if (file.exists()) {
                if (moveToTrash) {
                    ...
                    android.util.Log.d("RecordingRepository", "File deleted: $deleted, path: ${file.absolutePath}")
                }
                // 목록 다시 로드
                loadRecordings()
            }
