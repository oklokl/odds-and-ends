...
class ClockService : Service() {
    ...
    // ===== 상태 동기화 =====
    private val actionTimerState = "com.krdonon.timer.action.TIMER_STATE"
    private val extraState = "state"
    private val extraRemainMs = "remain_ms"
    private val stateRunning = "RUNNING"
    private val statePaused  = "PAUSED"
    private val stateStopped = "STOPPED"
    private val stateFinished = "FINISHED"

    // ===== 지속 저장용 SharedPreferences =====
    private val persistPrefs = "clock_persist_prefs"
    private val keyTimerEndElapsed = "timer_end_elapsed"
    private val keyTimerPaused = "timer_paused"
    private val keyTimerPausedRemain = "timer_paused_remain"
    private val keyStopwatchBase = "stopwatch_base"
    private val keyStopwatchRunning = "stopwatch_running"
    private val keyExtraTimersJson = "extra_timers_json"

    // ===== Keep-Alive 알람 =====
    private val keepAliveIntervalMs = 30 * 60 * 1000L
    private val keepAliveRequestCode = 99999

    private val syncPrefs = "clock_sync_prefs"
    private val keyLastState = "key_state"
    private val keyLastRemain = "key_remain_ms"
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START_TIMER -> {
                ...
                publishTimerState(stateRunning, durationMs, force = true)
            }
            ACTION_STOP_TIMER   -> {
                stopTimer()
                publishTimerState(stateStopped, 0L, force = true)
            }
            ACTION_PAUSE_TIMER  -> {
                pauseTimer()
                publishTimerState(statePaused, pausedRemainingMs, force = true)
            }
            ACTION_RESUME_TIMER -> {
                if (isTimerPaused && pausedRemainingMs > 0L) {
                    publishTimerState(stateRunning, pausedRemainingMs, force = true)
                }
                resumeTimer()
            }
    ...
    private fun persistTimerState() {
        getSharedPreferences(persistPrefs, Context.MODE_PRIVATE).edit {
            putLong(keyTimerEndElapsed, timerEndElapsed)
            putBoolean(keyTimerPaused, isTimerPaused)
            putLong(keyTimerPausedRemain, pausedRemainingMs)
        }
    }

    private fun clearTimerState() {
        getSharedPreferences(persistPrefs, Context.MODE_PRIVATE).edit {
            remove(keyTimerEndElapsed)
            remove(keyTimerPaused)
            remove(keyTimerPausedRemain)
        }
    }

    private fun persistStopwatchState() {
        getSharedPreferences(persistPrefs, Context.MODE_PRIVATE).edit {
            putLong(keyStopwatchBase, stopwatchBase)
            putBoolean(keyStopwatchRunning, stopwatchJob != null)
        }
    }

    private fun clearStopwatchState() {
        getSharedPreferences(persistPrefs, Context.MODE_PRIVATE).edit {
            remove(keyStopwatchBase)
            remove(keyStopwatchRunning)
        }
    }
    ...
    private fun persistExtraTimers() {
        ...
        getSharedPreferences(persistPrefs, Context.MODE_PRIVATE).edit {
            putString(keyExtraTimersJson, jsonArray.toString())
        }
    }

    private fun clearExtraTimers() {
        getSharedPreferences(persistPrefs, Context.MODE_PRIVATE).edit {
            remove(keyExtraTimersJson)
        }
    }

    private fun restorePersistedState() {
        val prefs = getSharedPreferences(persistPrefs, Context.MODE_PRIVATE)

        val savedEndElapsed = prefs.getLong(keyTimerEndElapsed, 0L)
        val wasPaused = prefs.getBoolean(keyTimerPaused, false)
        val savedPausedRemain = prefs.getLong(keyTimerPausedRemain, 0L)
        ...
        val swBase = prefs.getLong(keyStopwatchBase, 0L)
        val swRunning = prefs.getBoolean(keyStopwatchRunning, false)
        if (swRunning && swBase > 0L) {
            stopwatchBase = swBase
            startStopwatchJob()
        }

        val extrasJson = prefs.getString(keyExtraTimersJson, null)
    ...
    private fun startTimerJob() {
        ...
        timerJob = scope.launch {
            ...
            while (isActive) {
                ...
                if (remain <= 0L) {
                    // 🔹 알람 소리/진동 시작 (보조 타이머와 동일하게)
                    com.krdonon.timer.alarm.AlarmService.start(this@ClockService, "메인 타이머")

                    notifyTimer(0L)
                    publishTimerState(stateFinished, 0L, force = true)
                    delay(300)
    ...
    private fun scheduleKeepAliveAlarm() {
        ...
        val pi = PendingIntent.getService(
            this,
            keepAliveRequestCode,
            intent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )

        val triggerAt = System.currentTimeMillis() + keepAliveIntervalMs
    ...
    private fun publishTimerState(state: String, remainMs: Long, force: Boolean = false) {
        getSharedPreferences(syncPrefs, Context.MODE_PRIVATE).edit {
            putString(keyLastState, state)
            putLong(keyLastRemain, remainMs.coerceAtLeast(0L))
        }

        val intent = Intent(actionTimerState).apply {
            putExtra(extraState, state)
            putExtra(extraRemainMs, remainMs.coerceAtLeast(0L))
        }
    ...
    private fun publishTimerTickIfNeeded(remainMs: Long) {
        ...
        if (now - lastPublishMs >= 1000L) {
            publishTimerState(stateRunning, remainMs, force = false)
            lastPublishMs = now
        }
    }
