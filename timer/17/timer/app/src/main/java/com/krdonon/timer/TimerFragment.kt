...
class TimerFragment : Fragment() {
    ...
    private val timerStateReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (intent.action != actionTimerState) return

            val state = intent.getStringExtra(extraState) ?: return
            val remain = intent.getLongExtra(extraRemainMs, 0L).coerceAtLeast(0L)
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        btnAlertMode.setOnClickListener {
            ...
            alarmPrefs.edit { putString(AlarmService.KEY_ALERT_MODE, currentMode) }
            updateAlertModeButton()
        }
        ...
        btn10.setOnClickListener { setDurationAndPreview(java.util.concurrent.TimeUnit.MINUTES.toMillis(10)) }
        btn15.setOnClickListener { setDurationAndPreview(java.util.concurrent.TimeUnit.MINUTES.toMillis(15)) }
        btn30.setOnClickListener { setDurationAndPreview(java.util.concurrent.TimeUnit.MINUTES.toMillis(30)) }
    ...
    override fun onStart() {
        super.onStart()
        // 브로드캐스트 등록
        val filter = IntentFilter(actionTimerState)
    ...
    private fun syncFromServiceStateIfAny() {
        ...
        if (state != null) {
            when (state) {
                ...
                stateStopped, stateFinished -> resetMain()
            }
        }
    }
