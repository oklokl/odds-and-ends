...
class ClockService : Service() {
    ...
    private fun restorePersistedState() {
        ...
        if (wasPaused && savedPausedRemain > 0L) {
            ...
            val remain = savedEndElapsed - System.currentTimeMillis()  // 🔹 시스템 시간 기반으로 변경
            if (remain > 0L) {
        ...
        if (!extrasJson.isNullOrBlank()) {
            try {
                ...
                for (i in 0 until jsonArray.length()) {
                    ...
                    val remain = endElapsed - System.currentTimeMillis()  // 🔹 시스템 시간 기반으로 변경
                    if (remain > 0L) {
    ...
    private fun startTimer(durationMs: Long) {
        ensureChannels()
        timerEndElapsed = System.currentTimeMillis() + durationMs  // 🔹 시스템 시간 기반으로 변경
        persistTimerState()
        ensureForeground(Leader.TIMER)
        startTimerJob()
        scheduleKeepAliveAlarm()
    ...
    private fun startTimerJob() {
        ...
        timerJob = scope.launch {
            var last = 0L
            while (isActive) {
                val remain = timerEndElapsed - System.currentTimeMillis()  // 🔹 시스템 시간 기반으로 변경
                if (remain <= 0L) {
    ...
    private fun pauseTimer() {
        ...
        pausedRemainingMs = (timerEndElapsed - System.currentTimeMillis()).coerceAtLeast(0L)  // 🔹 시스템 시간 기반으로 변경
        isTimerPaused = true
        timerJob?.cancel()
        timerJob = null
        persistTimerState()
    ...
    private fun startExtra(id: String, label: String, durationMs: Long) {
        val endElapsed = System.currentTimeMillis() + durationMs  // 🔹 시스템 시간 기반으로 변경
        startExtraFromEndElapsed(id, label, endElapsed)
    }
    ...
    private fun startExtraFromEndElapsed(id: String, label: String, endElapsed: Long) {
        ...
        state.job = scope.launch {
            var last = 0L
            while (isActive) {
                val remain = state.endElapsed - System.currentTimeMillis()  // 🔹 시스템 시간 기반으로 변경
                if (remain <= 0L) {
                    // 🔹 알람 소리/진동 시작
    ...
    private fun onChannelPossiblyIdle() {
        ...
        when {
            timerRunning || isTimerPaused -> {
                ...
                else (timerEndElapsed - System.currentTimeMillis())  // 🔹 시스템 시간 기반으로 변경
                runCatching {
