    ...
    private val timerStateReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (isProcessingBroadcast) {
                android.util.Log.d("TimerFragment", "⚠️ 브로드캐스트 처리 중 - 스킵")
                return
            }
            ...
            try {
                when (state) {
                    ...
                    STATE_RUNNING -> {
                        // ✅ 이미 완전히 정지된 상태면 무시 (늦게 도착한 브로드캐스트)
                        if (timeLeftInMillis == 0L && !running) {
                            android.util.Log.d("TimerFragment", "⚠️ 이미 정지된 상태 - STATE_RUNNING 무시")
                            return
                        }
                ...
                // ✅ 100ms 후 플래그 해제 (중복 방지)
                handler.postDelayed({ isProcessingBroadcast = false }, 100)
            }
        }
    }
