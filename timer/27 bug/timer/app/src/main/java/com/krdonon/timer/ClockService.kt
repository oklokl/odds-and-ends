...
class ClockService : Service() {
    ...
    private fun restorePersistedState() {
        ...
        if (!extrasJson.isNullOrBlank()) {
            try {
                ...
                val validTimers = mutableListOf<ExtraTimerData>()
                ...
                for (i in 0 until jsonArray.length()) {
                    ...
                    // ✅ 남은 시간이 1초 이상인 경우만 복원
                    if (remain > 1000L) {
                        startExtraFromEndElapsed(id, label, endElapsed)
                        validTimers.add(ExtraTimerData(id, label, endElapsed))
                    } else {
                        android.util.Log.d("ClockService", "⚠️ 만료된 타이머 무시: $id, remain=$remain")
                    }
                }

                // ✅ 유효한 타이머만 다시 저장 (만료된 타이머 제거)
                if (validTimers.isEmpty()) {
                    clearExtraTimers()
                } else if (validTimers.size < jsonArray.length()) {
                    // 일부 타이머가 만료되었으면 유효한 타이머만 다시 저장
                    ...
                    prefs.edit().putString(KEY_EXTRA_TIMERS_JSON, newJsonArray.toString()).apply()
                }
            } catch (e: Exception) {
                e.printStackTrace()
                // 파싱 오류 시 전체 삭제
    ...
    // ✅ 보조 타이머 데이터 클래스 추가
    private data class ExtraTimerData(val id: String, val label: String, val endElapsed: Long)
    ...
    private fun startTimerJob() {
        ...
        timerJob = scope.launch {
            var last = 0L
            while (isActive) {
                // ✅ 일시정지 또는 정지 상태면 Job 종료
                if (isTimerPaused || timerJob == null) {
                    android.util.Log.d("ClockService", "✅ 타이머 정지 감지 - Job 종료")
                    break
                }
    ...
    private fun startExtraFromEndElapsed(id: String, label: String, endElapsed: Long) {
        ...
        state.job = scope.launch {
            ...
            while (isActive) {
                ...
                if (remain <= 0L) {
                    android.util.Log.d("ClockService", "✅ 보조 타이머 종료: $id ($label)")
                    ...
                    // ✅ SharedPreferences 즉시 업데이트
                    persistExtraTimers()

                    if (extraTimers.isEmpty()) {
                        cancelSafe(EXTRA_SUMMARY_ID)
                        clearExtraTimers()
                        android.util.Log.d("ClockService", "✅ 모든 보조 타이머 종료 - Keep-Alive 취소 예정")
                    } else {
                        postOrUpdateExtraSummary(force = true)
                    }
    ...
    private fun onChannelPossiblyIdle() {
        ...
        when {
            ...
            stopwatchRunning -> {
                ...
                // ✅ 모든 타이머가 종료되었으므로 Keep-Alive 알람 취소
                cancelKeepAliveAlarm()
    ...
    private fun publishTimerState(state: String, remainMs: Long, force: Boolean = false) {
        ...
        android.util.Log.d("ClockService", "✅ 브로드캐스트 전송: state=$state, remain=$remain, thread=${Thread.currentThread().name}")
    }
