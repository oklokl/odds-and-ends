...
class StopWatchFragment : Fragment() {
    ...
    /**
     * 2번째 스톱워치는 세로 화면 안에서 QuarterTurnLayout으로 90도 회전해
     * 가로 사용 형태를 만드는 화면입니다.
     *
     * 따라서 시스템 자동 회전까지 동시에 적용되면 화면이 한 번 더 돌아가
     * 가로 상태에서 레이아웃이 좁고 세로로 보이게 됩니다.
     * 2번째 스톱워치가 실제로 보이는 동안에는 Activity를 세로 방향으로 유지하고,
     * 이 화면을 벗어나면 다시 시스템의 회전 설정을 따르도록 복원합니다.
     */
    ...
    private fun applyStopwatchMode() {
        if (showingSecond) {
            ...
            // 숨겨진 스톱워치 탭이 다른 탭의 화면 방향까지 강제로 바꾸지 않도록
            // 실제로 이 Fragment가 보이는 경우에만 세로 방향을 고정합니다.
            if (!isHidden) {
