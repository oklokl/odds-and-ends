...
class TimerFragment : Fragment() {
    ...
    // 🔹 보조 타이머의 종료 시각을 저장 (System.currentTimeMillis() 기준으로 변경)
    ...
    private fun addBlockTimerView(state: TimerViewModel.ExtraTimer) {
        ...
        // 🔹 핵심: 종료 시각 기준으로 남은 시간 계산 (System.currentTimeMillis() 기반)
        val updateRunnable = object : Runnable {
            override fun run() {
                if (state.running) {
                    // 🔹 종료 시각에서 현재 시각을 빼서 남은 시간 계산 (System.currentTimeMillis() 사용)
                    val endTime = extraTimerEndTimes[state.id] ?: 0L
                    if (endTime > 0L) {
                        val remain = (endTime - System.currentTimeMillis()).coerceAtLeast(0L)  // 🔹 수정
                        ...
                        if (remain <= 0L) {
                            ...
                            // 🔹 AlarmService.start() 제거 - Service에서 이미 울림!
        ...
        btnStart.setOnClickListener {
            if (state.running) {
                ...
                // 🔹 종료 시각 저장 (현재 시각 + 남은 시간) - System.currentTimeMillis() 사용
                extraTimerEndTimes[state.id] = System.currentTimeMillis() + state.remainingMs  // 🔹 수정
        ...
        if (state.running && state.remainingMs > 0L) {
            // 🔹 종료 시각 복원 - System.currentTimeMillis() 사용
            extraTimerEndTimes[state.id] = System.currentTimeMillis() + state.remainingMs  // 🔹 수정
