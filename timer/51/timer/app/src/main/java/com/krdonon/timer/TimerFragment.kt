...
class TimerFragment : Fragment() {
    ...
    private fun syncExtraTimersWithService() {
        ...
        if (extrasJson.isNullOrBlank()) {
            // 서비스에 실행중인 보조 타이머가 없으면: ViewModel에서 running만 일시정지로 내려준다(remaining은 유지)
            viewModel.extras.value?.forEach { timer ->
                if (timer.running) {
                    timer.running = false
                    viewModel.setRunning(timer.id, false)
        ...
        runCatching {
            ...
            for (i in 0 until arr.length()) {
                ...
                val endAtMs = o.optLong("endElapsed", 0L)
                ...
                if (remaining > 0L) {
                    ...
                    viewModel.upsertExtraFromService(id, label, 0L, false)
                }
            }

            // 서비스에 없는 running 타이머는 '늘리지' 않도록 일시정지로 전환
            viewModel.extras.value?.forEach { timer ->
                if (timer.running && !serviceIds.contains(timer.id)) {
                    timer.running = false
                    viewModel.setRunning(timer.id, false)
                    extraTimerEndTimes.remove(timer.id)
                }
