...
class ClockService : Service() {
    ...
    // 정시(targetAt) 타이머는 wall-clock 기준으로 만료를 판단해야 정시보다 일찍 울리지 않는다.
    private var timerUsesWallClockDeadline: Boolean = false
    ...
    private val KEY_TIMER_USE_WALL_CLOCK = "timer_use_wall_clock"
    ...
    private suspend fun handleAction(intent: Intent?) {
        when (intent?.action) {
            ...
            ACTION_START_TIMER -> {
                ...
                val targetAtWallMs = intent.getLongExtra(EXTRA_TARGET_AT_WALL, -1L)
                ...
                withContext(Dispatchers.Main) {
                    ...
                    delay(50)
                    val remainForBroadcast = if (timerEndAtWall > 0L) {
                        (timerEndAtWall - System.currentTimeMillis()).coerceAtLeast(0L)
                    } else {
                        durationMs
                    }
                    publishTimerState(STATE_RUNNING, remainForBroadcast, force = true)
                }
            }
            ACTION_STOP_TIMER -> {
            ...
            ACTION_START_EXTRA -> {
                ...
                val totalMs = intent.getLongExtra(EXTRA_TOTAL_DURATION_MS, ms).coerceAtLeast(ms)
                withContext(Dispatchers.Main) {
                    startExtra(id, label, ms, totalMs)
                }
            }
            ACTION_STOP_EXTRA -> {
                val id = intent.getStringExtra(EXTRA_ID) ?: return
    ...
    private fun persistTimerState() {
        getSharedPreferences(PERSIST_PREFS, Context.MODE_PRIVATE).edit()
            ...
            .putBoolean(KEY_TIMER_USE_WALL_CLOCK, timerUsesWallClockDeadline)
    ...
    private fun clearTimerState() {
        getSharedPreferences(PERSIST_PREFS, Context.MODE_PRIVATE).edit()
            ...
            .remove(KEY_TIMER_USE_WALL_CLOCK)
    ...
    private fun restorePersistedState() {
        ...
        val savedUseWallClock = prefs.getBoolean(KEY_TIMER_USE_WALL_CLOCK, false)
        ...
        if (wasPaused && savedPausedRemain > 0L) {
            ...
            timerUsesWallClockDeadline = savedUseWallClock
            isTimerPaused = true
            ...
            if (remain > 0L) {
                ...
                timerSessionId = savedSessionId
                timerUsesWallClockDeadline = false
                clearTimerState()
        ...
        if (swRunning && swBase > 0L) {
            ...
            notifyStopwatch(currentStopwatchElapsedMs())
            startStopwatchJob()
        }
        ...
        AppLog.refreshIdleCleanupPolicy(applicationContext)
    }
    ...
    private fun startTimerAt(targetAtWallMs: Long) {
        ...
        timerUsesWallClockDeadline = true
        persistTimerState()
        AppLog.refreshIdleCleanupPolicy(applicationContext)
        startTimerJob()
        ensureForeground(Leader.TIMER)
    ...
    private fun startTimerJob() {
        ...
        timerJob = scope.launch {
            ...
            while (isActive) {
                ...
                val remain = if (timerUsesWallClockDeadline && timerEndAtWall > 0L) {
                    timerEndAtWall - System.currentTimeMillis()
                } else {
                    timerEndElapsed - nowElapsed
                }
                if (remain <= 0L) {
                    ...
                    val t = nowElapsed
                    if (t - last >= 1000) {
                        // RemoteViews Chronometer가 SystemUI에서 스스로 카운트다운하므로
                        // 매초 notify()로 알림 전체를 다시 만들 필요가 없다.
                        // 반복 notify는 다른 ongoing 알림(스톱워치)과 레이아웃 애니메이션 충돌을 만든다.
                        persistTimerState()
                        last = t
                    }
    ...
    private fun resumeTimer() {
        ...
        val resumeRemainingMs = pausedRemainingMs
        isTimerPaused = false

        ensureChannels()
        // 🔹 총 기간 정보가 없으면(구버전 복원 등) 남은 시간을 총 기간으로 사용
        if (timerTotalDurationMs <= 0L) timerTotalDurationMs = resumeRemainingMs

        timerEndElapsed = SystemClock.elapsedRealtime() + resumeRemainingMs
        timerEndAtWall = System.currentTimeMillis() + resumeRemainingMs
        timerUsesWallClockDeadline = false
    ...
    private fun startStopwatch(baseElapsed: Long = 0L) {
        ...
        AppLog.refreshIdleCleanupPolicy(applicationContext)
        ensureForeground(Leader.STOPWATCH)
        notifyStopwatch(baseElapsed.coerceAtLeast(0L))
        startStopwatchJob()
    }
    ...
    private fun startStopwatchJob() {
        ...
        stopwatchJob = scope.launch {
            ...
            while (isActive) {
                ...
                if (t - last >= 1000) {
                    // 본문의 Chronometer가 SystemUI에서 직접 증가하므로 매초 알림을 재게시하지 않는다.
                    persistStopwatchState()
                    last = t
                }
                delay(100)
    ...
    private fun startStopwatch2(baseElapsed: Long = 0L) {
        ...
        AppLog.refreshIdleCleanupPolicy(applicationContext)
        ensureForeground(Leader.STOPWATCH2)
        notifyStopwatch2(baseElapsed.coerceAtLeast(0L))
        startStopwatch2Job()
    }
    ...
    // ===================== Extras (보조 타이머) =====================
    private fun startExtra(id: String, label: String, durationMs: Long, totalDurationMs: Long = durationMs) {
        ...
        startExtraFromEndElapsed(id, label, endElapsed, endAtWall, totalDurationMs)
    }
    ...
    // ===================== Foreground 유지 =====================
    /**
     * 메인 타이머와 스톱워치를 동시에 실행할 때는 메인 타이머를 Foreground 리더로 둔다.
     *
     * 한 Service 안에서 여러 진행 알림을 매초 번갈아 Foreground처럼 갱신하면 일부 SystemUI
     * (특히 Samsung One UI)에서 두 알림이 서로 확장/축소되거나 대표 알림이 바뀌는 것처럼
     * 보일 수 있다. Foreground 리더는 안정적으로 하나만 유지하고 나머지는 고유 ID의
     * 일반 ongoing 알림으로 유지한다.
     */
    private fun ensureForeground(preferred: Leader) {
        ...
        val initial: Notification = when (preferred) {
            Leader.TIMER -> buildTimerNotificationSafe(currentTimerRemainingMs())
            Leader.STOPWATCH -> buildStopwatchNotification(currentStopwatchElapsedMs())
            Leader.STOPWATCH2 -> buildStopwatch2Notification(currentStopwatch2ElapsedMs())
            Leader.EXTRA -> buildExtraSummaryForegroundNotification()
        }

        startOrSwitchForeground(preferred, initial)

        // 리더가 바뀌면서 제거된 기존 Foreground 알림은 고유한 일반 알림 ID로 즉시 복원한다.
        // 이렇게 하면 타이머(첫 번째)와 스톱워치(두 번째)가 서로 덮어쓰지 않고 독립적으로 남는다.
        when (previousLeader) {
            Leader.STOPWATCH -> if (stopwatchJob != null) {
                cancelSafe(FOREGROUND_ID_STOPWATCH)
                notifySafe(NID_STOPWATCH, buildStopwatchNotification(currentStopwatchElapsedMs()))
            }
            Leader.STOPWATCH2 -> if (stopwatch2Job != null) {
                cancelSafe(FOREGROUND_ID_STOPWATCH2)
                notifySafe(NID_STOPWATCH2, buildStopwatch2Notification(currentStopwatch2ElapsedMs()))
            }
            Leader.EXTRA -> if (extraTimers.isNotEmpty()) {
    ...
    private fun currentTimerRemainingMs(): Long = when {
        isTimerPaused -> pausedRemainingMs.coerceAtLeast(0L)
        timerUsesWallClockDeadline && timerEndAtWall > 0L ->
            (timerEndAtWall - System.currentTimeMillis()).coerceAtLeast(0L)
        timerEndElapsed > 0L ->
    ...
    private fun currentStopwatchElapsedMs(): Long =
        if (stopwatchBase > 0L) (SystemClock.elapsedRealtime() - stopwatchBase).coerceAtLeast(0L) else 0L

    private fun currentStopwatch2ElapsedMs(): Long =
        if (stopwatch2Base > 0L) (SystemClock.elapsedRealtime() - stopwatch2Base).coerceAtLeast(0L) else 0L
    ...
    private fun notifyTimer(remainingMs: Long) {
        // 타이머 알림은 ID를 하나(FOREGROUND_ID_TIMER)로 통일해서 중복 노출을 원천 차단
        cancelSafe(NID_TIMER)
        notifySafe(FOREGROUND_ID_TIMER, buildTimerNotificationSafe(remainingMs))
    }
    ...
    private fun buildTimerNotificationSafe(remainingMs: Long): Notification {
        return try {
            ...
            val deleteIntent = Intent(this, com.krdonon.timer.NotificationDismissReceiver::class.java).apply {
                action = com.krdonon.timer.NotificationDismissReceiver.ACTION_DISMISS_TIMER
            }
            val deletePi = PendingIntent.getBroadcast(
                this, 2101, deleteIntent,
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
            )
    ...
    /**
     * 스톱워치 알림의 시간은 SystemUI 상단 헤더 chronometer와 본문 텍스트를
     * 동시에 사용하지 않는다.
     *
     * 기존 방식은:
     *  - 헤더의 작은 chronometer = SystemUI가 직접 갱신하므로 정확함
     *  - 본문의 "경과 시간 ..." = 서비스 notify 주기(약 1초)로 갱신되어 1초가량 늦어 보임
     *
     * 그래서 헤더 시간은 숨기고(setShowWhen(false)), 같은 elapsedRealtime base를 사용하는
     * RemoteViews Chronometer를 본문에 크게 표시한다. 이 Chronometer는 SystemUI가 직접
     * 갱신하므로 기존 상단 작은 시간과 같은 기준으로 움직이며 중복 표시도 사라진다.
     */
    private fun buildRunningStopwatchNotification(
        title: String,
        elapsedMs: Long,
        stableBaseElapsed: Long,
        notificationGroup: String
    ): Notification {
        val nowElapsed = SystemClock.elapsedRealtime()
        val chronometerBase = if (stableBaseElapsed > 0L) {
            stableBaseElapsed
        } else {
            nowElapsed - elapsedMs.coerceAtLeast(0L)
        }

        val compact = RemoteViews(packageName, R.layout.notification_stopwatch_compact).apply {
            // 접힌 알림은 높이가 매우 제한적이므로 제목 + 시간을 한 줄로 배치한다.
            setTextViewText(R.id.stopwatch_notification_title, title)
            setChronometer(
        ...
        val expanded = RemoteViews(packageName, R.layout.notification_stopwatch_expanded).apply {
            // 펼친 알림의 본문에는 정확한 Chronometer만 크게 보여준다.
            // 제목은 setSubText(title)로 SystemUI 헤더의 앱 이름 옆에 표시한다.
            setChronometer(
                R.id.stopwatch_notification_chronometer,
                chronometerBase,
                "%s",
        ...
        val actionDismiss = if (title == "스톱워치 2") {
            com.krdonon.timer.NotificationDismissReceiver.ACTION_DISMISS_STOPWATCH2
        } else {
            com.krdonon.timer.NotificationDismissReceiver.ACTION_DISMISS_STOPWATCH
        }
        val reqCode = if (title == "스톱워치 2") 2103 else 2102
        val deleteIntent = Intent(this, com.krdonon.timer.NotificationDismissReceiver::class.java).apply {
            action = actionDismiss
        }
        val deletePi = PendingIntent.getBroadcast(
            this, reqCode, deleteIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )

        return NotificationCompat.Builder(this, STOPWATCH_CHANNEL)
            .setContentTitle(title)
            // 타이머와 명시적으로 다른 group key를 사용해 One UI의 자동 묶음/대표 알림 교체를 막는다.
            .setGroup(notificationGroup)
            // 펼친 상태에서 "내일 타이머 · 스톱워치"처럼 앱 이름 옆에 종류를 표시한다.
            .setSubText(title)
            .setSmallIcon(R.drawable.ic_notification)
            .setOngoing(true)
            .setOnlyAlertOnce(true)
            // 헤더의 작은 시간/chronometer는 표시하지 않는다.
            .setShowWhen(false)
    ...
    private fun buildStopwatchNotification(elapsedMs: Long): Notification =
        buildRunningStopwatchNotification(
            title = "스톱워치",
            elapsedMs = elapsedMs,
            stableBaseElapsed = stopwatchBase,
            notificationGroup = STOPWATCH_NOTIFICATION_GROUP
        )
    ...
    private fun buildStopwatch2Notification(elapsedMs: Long): Notification =
        buildRunningStopwatchNotification(
            title = "스톱워치 2",
            elapsedMs = elapsedMs,
            stableBaseElapsed = stopwatch2Base,
            notificationGroup = STOPWATCH2_NOTIFICATION_GROUP
        )
    ...
    private fun buildExtraNotification(id: String, label: String, remainingMs: Long): Notification {
        ...
        val deleteIntent = Intent(this, com.krdonon.timer.NotificationDismissReceiver::class.java).apply {
            action = com.krdonon.timer.NotificationDismissReceiver.ACTION_DISMISS_EXTRA
            putExtra(com.krdonon.timer.NotificationDismissReceiver.EXTRA_ID, id)
        }
        val deletePi = PendingIntent.getBroadcast(
            this, 2104 + (id.hashCode() and 0x7FFF), deleteIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )
    ...
    private fun publishTimerState(state: String, remainMs: Long, force: Boolean = false) {
        ...
        if (state == STATE_RUNNING) {
            AppLog.dProgress(
                applicationContext,
                "ClockService",
                "broadcast_sent_running",
                timerSessionId,
                remain,
                bucketSec = 15,
                finalCountdownSec = 5,
                message = "broadcast sent state=$state remain=$remain session=$timerSessionId"
            )
        } else {
    ...
    private fun formatEndAtKorean(endAtMillis: Long): String {
        ...
        val date = java.util.Date((endAtMillis / 60_000L) * 60_000L)
    ...
    fun onNotificationDismissed(type: String, extraId: String?) {
        scope.launch {
            delay(1000L) // 사용자가 스와이프한 후 잠시 뒤(1초) 활성 여부 판단 후 복원
            when (type) {
                DISMISS_TYPE_TIMER -> {
                    val running = (timerJob != null) && !isTimerPaused
                    val active = running || isTimerPaused
                    if (active) {
                        val remain = currentTimerRemainingMs()
                        if (running || remain > 0L) {
                            AppLog.i(applicationContext, "ClockService", "Timer notification was dismissed while active; restoring")
                            notifyTimer(remain)
                        }
                    }
                }
                DISMISS_TYPE_STOPWATCH -> {
                    if (stopwatchJob != null) {
                        AppLog.i(applicationContext, "ClockService", "Stopwatch notification was dismissed while running; restoring")
                        notifyStopwatch(currentStopwatchElapsedMs())
                    }
                }
                DISMISS_TYPE_STOPWATCH2 -> {
                    if (stopwatch2Job != null) {
                        AppLog.i(applicationContext, "ClockService", "Stopwatch2 notification was dismissed while running; restoring")
                        notifyStopwatch2(currentStopwatch2ElapsedMs())
                    }
                }
                DISMISS_TYPE_EXTRA -> {
                    val now = SystemClock.elapsedRealtime()
                    if (extraId != null) {
                        val timer = extraTimers[extraId]
                        if (timer != null && timer.job != null && timer.endElapsed > now) {
                            val remain = (timer.endElapsed - now).coerceAtLeast(0L)
                            AppLog.i(applicationContext, "ClockService", "Extra timer notification was dismissed while running; restoring: $extraId")
                            notifySafe(extraNotifyId(timer.id), buildExtraNotification(timer.id, timer.label, remain))
                        }
                    } else {
                        val runningExtras = extraTimers.values.filter { it.job != null && it.endElapsed > now }
                        if (runningExtras.isNotEmpty()) {
                            runningExtras.forEach { timer ->
                                val remain = (timer.endElapsed - now).coerceAtLeast(0L)
                                notifySafe(extraNotifyId(timer.id), buildExtraNotification(timer.id, timer.label, remain))
                            }
                        }
    ...
    companion object {
        const val DISMISS_TYPE_TIMER = "TIMER"
        const val DISMISS_TYPE_STOPWATCH = "STOPWATCH"
        const val DISMISS_TYPE_STOPWATCH2 = "STOPWATCH2"
        const val DISMISS_TYPE_EXTRA = "EXTRA"

        private var instance: ClockService? = null

        fun handleNotificationDismissed(type: String, extraId: String? = null) {
            instance?.onNotificationDismissed(type, extraId)
        }
        ...
        private const val EXTRA_TOTAL_DURATION_MS = "total_duration_ms"
        private const val EXTRA_TARGET_AT_WALL = "target_at_wall"
        ...
        // NOTE: Foreground/Background를 다른 ID로 쓰면 실기기에서 "타이머 알림 2개"처럼 보일 수 있어 동일 ID로 통일
        // 타이머만 Foreground ID와 통일해 중복 알림을 막고,
        // 스톱워치는 별도 일반 알림 ID를 유지해서 타이머와 동시에 표시되도록 한다.
        ...
        // 메인 타이머와 스톱워치는 같은 앱의 알림이지만 서로 다른 명시적 group key를 사용한다.
        // 일부 SystemUI가 동일 앱의 ongoing 알림을 자동으로 한 묶음처럼 처리하며 대표 알림을
        // 번갈아 바꾸는 현상을 방지하기 위한 값이다.
        private const val TIMER_NOTIFICATION_GROUP = "main_timer_notification_group"
        private const val STOPWATCH_NOTIFICATION_GROUP = "stopwatch_notification_group"
        private const val STOPWATCH2_NOTIFICATION_GROUP = "stopwatch2_notification_group"
        ...
        private const val KEY_TIMER_END_ELAPSED_STATIC = "timer_end_elapsed"
        private const val KEY_TIMER_PAUSED_REMAIN_STATIC = "timer_paused_remain"
        private const val KEY_STOPWATCH_RUNNING_STATIC = "stopwatch_running"
        private const val KEY_STOPWATCH2_RUNNING_STATIC = "stopwatch2_running"

        fun hasActiveWork(context: Context): Boolean {
            val prefs = context.getSharedPreferences(PERSIST_PREFS_STATIC, Context.MODE_PRIVATE)
            val timerEndElapsed = prefs.getLong(KEY_TIMER_END_ELAPSED_STATIC, 0L)
            val pausedRemain = prefs.getLong(KEY_TIMER_PAUSED_REMAIN_STATIC, 0L)
            val stopwatchRunning = prefs.getBoolean(KEY_STOPWATCH_RUNNING_STATIC, false)
            val stopwatch2Running = prefs.getBoolean(KEY_STOPWATCH2_RUNNING_STATIC, false)
            val extrasJson = prefs.getString(KEY_EXTRA_TIMERS_JSON_STATIC, null)

            val hasMainTimer = timerEndElapsed > 0L || pausedRemain > 0L
            val hasExtraTimers = !extrasJson.isNullOrBlank() && extrasJson != "[]"
            return hasMainTimer || hasExtraTimers || stopwatchRunning || stopwatch2Running
        }
        ...
        fun startTimerAt(context: Context, targetAtWallMs: Long) {
            val i = Intent(context, ClockService::class.java).apply {
                action = ACTION_START_TIMER
                putExtra(EXTRA_TARGET_AT_WALL, targetAtWallMs)
                putExtra(EXTRA_DURATION_MS, (targetAtWallMs - System.currentTimeMillis()).coerceAtLeast(0L))
            }
            ContextCompat.startForegroundService(context, i)
        }
        ...
        fun startExtraTimer(context: Context, id: String, label: String, durationMs: Long, totalDurationMs: Long = durationMs) {
            val i = Intent(context, ClockService::class.java).apply {
                action = ACTION_START_EXTRA
                putExtra(EXTRA_ID, id)
                putExtra(EXTRA_LABEL, label)
                putExtra(EXTRA_DURATION_MS, durationMs)
                putExtra(EXTRA_TOTAL_DURATION_MS, totalDurationMs.coerceAtLeast(durationMs))
            }
            context.startService(i)
        }
