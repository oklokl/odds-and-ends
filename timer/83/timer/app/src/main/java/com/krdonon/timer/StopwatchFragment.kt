...
class StopWatchFragment : Fragment() {
    ...
    /**
     * 스톱워치 탭은 클래식/2번째 화면 모두 Activity 자체는 세로 방향으로 유지합니다.
     *
     * - 클래식 화면: 시스템 자동 회전 사용자가 휴대폰을 가로로 기울여도
     *   별도의 landscape 레이아웃으로 재배치하지 않고, 사용자가 원하는 것처럼
     *   기존 세로 스톱워치 화면을 그대로 유지합니다.
     * - 2번째 화면: QuarterTurnLayout이 세로 Activity 안에서 UI를 90도 회전하므로,
     *   시스템 회전까지 동시에 적용되어 이중 회전되는 문제를 막습니다.
     *
     * 스톱워치 탭을 벗어날 때는 반드시 UNSPECIFIED로 복원하여
     * 알림/타이머 화면이 사용자의 시스템 회전 설정을 다시 따르게 합니다.
     */
    private fun lockHostToPortraitForStopwatch() {
    ...
    private fun applyStopwatchMode() {
        ...
        // 클래식/2번째 스톱워치 모두 동일하게 세로 Activity를 유지합니다.
        // 단, 숨겨진 스톱워치 탭이 다른 탭의 화면 방향까지 강제로 바꾸지 않도록
        // 실제로 이 Fragment가 보이는 경우에만 적용합니다.
        if (!isHidden) {
            lockHostToPortraitForStopwatch()
        }
    }
    ...
    override fun onHiddenChanged(hidden: Boolean) {
        ...
        // 하단 탭을 바꿨을 때 스톱워치의 세로 고정이
        // 타이머/알림 화면에 남지 않도록 즉시 원복합니다.
        if (hidden) {
            restoreHostOrientationToSystem()
        } else {
            // 클래식 화면과 2번째 화면 모두 동일하게 세로 Activity를 유지합니다.
            lockHostToPortraitForStopwatch()
        }
    }
