...
class NumberPadFragment : Fragment() {
    ...
    private var isExtraMode: Boolean = false
    ...
    private var nowBaseWallTimeMs: Long = 0L
    private var nowBaseElapsedTimeMs: Long = 0L

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?
    ): View {
        isExtraMode = arguments?.getBoolean(ARG_IS_EXTRA_MODE, false) == true
        val v = inflater.inflate(R.layout.fragment_number_pad, container, false)
        ...
        if (isExtraMode) {
            // 보조 타이머도 메인과 동일하게 절대 시각(내일/오전·오후) 입력을 허용한다.
            // UI 크기는 그대로 두고 완료 버튼만 "추가"로 바꾼다.
            v.findViewById<Button>(R.id.btnNext).text = getString(R.string.btn_add)
        }
        ...
        bindPreset(v, R.id.btnPreset5, 5)
        bindPreset(v, R.id.btnPreset10, 10)
        bindPreset(v, R.id.btnPreset20, 20)
        bindPreset(v, R.id.btnPreset30, 30)
        bindPreset(v, R.id.btnPreset40, 40)
        bindPreset(v, R.id.btnPreset50, 50)
        ...
        // 상단 현재 시각: wall clock을 elapsedRealtime에 고정해 부드럽게 갱신
        // 기존 100ms 폴링은 밀리초 3자리가 끊겨 보이고 특정 자리 숫자가 늦게 변하는 듯한 인상을 준다.
        syncNowClockBase()
    ...
    private fun bindPreset(root: View, buttonId: Int, minutes: Int) {
        root.findViewById<Button>(buttonId).setOnClickListener {
            applyDurationPreset(minutes)
        }
    }

    private fun applyDurationPreset(minutes: Int) {
        isTomorrow = false
        ampmSelected = false
        isAm = true
        btnTomorrow.text = getString(R.string.label_tomorrow)
        btnAmPm.text = getString(R.string.label_am)
        btnAmPm.setTextColor(labelDefaultColor)
        ...
        val hText = String.format(Locale.getDefault(), "%02d", hours)
        val mText = String.format(Locale.getDefault(), "%02d", mins)
        val sText = String.format(Locale.getDefault(), "%02d", secs)
    ...
    private fun updateNowOnPad() {
        val estimatedWallTime = nowBaseWallTimeMs + (SystemClock.elapsedRealtime() - nowBaseElapsedTimeMs)
        val now = Calendar.getInstance().apply { timeInMillis = estimatedWallTime }
        ...
        // wall clock과의 오차가 누적되지 않도록 주기적으로 기준점을 재동기화
        if (kotlin.math.abs(System.currentTimeMillis() - estimatedWallTime) >= 250L) {
            syncNowClockBase()
        }
    }

    private fun syncNowClockBase() {
        nowBaseWallTimeMs = System.currentTimeMillis()
        nowBaseElapsedTimeMs = SystemClock.elapsedRealtime()
    }
    ...
    private fun finishAndReturn() {
        ...
        if (isTomorrow) {
            val target = Calendar.getInstance().apply {
                ...
                // 시각 지정 모드는 UI가 사실상 시/분 예약으로 동작하므로 초/밀리초를 버린다.
                set(Calendar.SECOND, 0)
                set(Calendar.MILLISECOND, 0)
            }
            AppLog.i(requireContext(), "NumberPad", "result tomorrow targetAt=${target.timeInMillis} h=$h m=$m rawS=$s rawMs=$ms")
            ...
            val target = Calendar.getInstance().apply {
                set(Calendar.HOUR_OF_DAY, h)
                set(Calendar.MINUTE, m)
                // 오전/오후 시각 지정도 절대 시각으로 넘기고 초/밀리초는 0으로 고정한다.
                set(Calendar.SECOND, 0)
                set(Calendar.MILLISECOND, 0)
            }
            if (target.before(now)) target.add(Calendar.DAY_OF_YEAR, 1)
            AppLog.i(requireContext(), "NumberPad", "result ampm targetAt=${target.timeInMillis} h=$h m=$m rawS=$s rawMs=$ms")
            bundle.putLong("targetAt", target.timeInMillis)
            ...
            val duration =
                TimeUnit.HOURS.toMillis(h.toLong()) +
                        ...
                        ms.toLong()
            AppLog.i(requireContext(), "NumberPad", "result duration durationMs=${max(0L, duration)} h=$h m=$m s=$s ms=$ms")
            bundle.putLong("duration", max(0L, duration))
        }

        bundle.putBoolean(ARG_IS_EXTRA_MODE, isExtraMode)
    ...
    companion object {
        const val RESULT_KEY = "numberPadResult"
        const val ARG_IS_EXTRA_MODE = "is_extra_mode"

        fun newInstance(isExtraMode: Boolean = false): NumberPadFragment {
            return NumberPadFragment().apply {
                arguments = Bundle().apply {
                    putBoolean(ARG_IS_EXTRA_MODE, isExtraMode)
                }
            }
        }
    }
