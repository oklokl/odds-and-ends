...
import com.krdonon.timer.AppLog
...
class AlarmService : Service() {
    ...
    companion object {
        ...
        const val KEY_RINGING_LABEL = "ringing_label"
        ...
        fun stop(context: Context) {
            ...
            try {
                // ✅ deprecated된 getRunningServices() 제거
                // startService는 서비스가 실행 중이 아니면 자동으로 무시됨
        ...
        fun forceStop(context: Context) {
            ...
            runCatching {
                context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
                    .edit().putBoolean(KEY_RINGING, false).remove(KEY_RINGING_LABEL).apply()
            }
        }

        private var instance: AlarmService? = null

        fun handleNotificationDismissed(context: Context, label: String) {
            instance?.onNotificationDismissed(label)
        }
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        AppLog.d(applicationContext, "AlarmService", "onStartCommand action=${intent?.action} label=${intent?.getStringExtra(EXTRA_LABEL)}")
        when (intent?.action) {
    ...
    fun onNotificationDismissed(label: String) {
        val prefs = getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val isRinging = prefs.getBoolean(KEY_RINGING, false)
        if (isRinging) {
            AppLog.i(applicationContext, "AlarmService", "Alarm notification was dismissed while ringing; restoring after 1s")
            stopHandler.postDelayed({
                val stillRinging = getSharedPreferences(PREFS, Context.MODE_PRIVATE).getBoolean(KEY_RINGING, false)
                if (stillRinging) {
                    val notification = buildRingingNotification(label)
                    runCatching {
    ...
    private fun buildRingingNotification(label: String): Notification {
        ...
        val deleteIntent = Intent(this, com.krdonon.timer.NotificationDismissReceiver::class.java).apply {
            action = com.krdonon.timer.NotificationDismissReceiver.ACTION_DISMISS_ALARM_TIMER
            putExtra(com.krdonon.timer.NotificationDismissReceiver.EXTRA_LABEL, label)
        }
        ...
        val builder = NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .setOngoing(true)
            .setAutoCancel(false)
            .addAction(R.drawable.ic_stop, "알람 끄기", stopPi)
            .setDeleteIntent(deletePi)

        if (isLocked) {
            // 잠금 상태에서만 풀스크린으로 깨워 띄움
    ...
    private fun startAlarm(label: String) {
        ...
        val currentLabel = prefs.getString(KEY_RINGING_LABEL, null)
        if (alreadyRinging && currentLabel == label) {
            AppLog.d(applicationContext, "AlarmService", "startAlarm ignored duplicate label=$label")
            return
        }

        AppLog.i(applicationContext, "AlarmService", "startAlarm label=$label")
        ...
        prefs.edit()
            ...
            .putString(KEY_RINGING_LABEL, label)
            .apply()
        ...
        // - 설정: 5~60분
        // - 연속: 자동 종료 없음
        val forever = prefs.getBoolean(TimerAlarmPrefs.KEY_TIMER_RING_FOREVER, false)
        val minutes = prefs.getInt(TimerAlarmPrefs.KEY_TIMER_RING_DURATION_MINUTES, 5)
            .coerceIn(5, 60)
        if (!forever) {
            AppLog.d(applicationContext, "AlarmService", "auto stop scheduled after ${minutes}m")
            stopHandler.postDelayed({
                AppLog.d(applicationContext, "AlarmService", "auto stop fired after ${minutes}m")
                stopAlarm()
            }, minutes * 60 * 1000L)
        } else {
            AppLog.d(applicationContext, "AlarmService", "ring forever enabled")
        }

        // 종료 시각 기록 알림은 유지하되, 동일 알람 중복 기록은 내부에서 차단한다
        postHistoryNotification(label)
    }

    private fun stopAlarm() {
        AppLog.i(applicationContext, "AlarmService", "stopAlarm")
        stopHandler.removeCallbacksAndMessages(null)
        stopSound()
        stopVibration()
    ...
    override fun onDestroy() {
        AppLog.i(applicationContext, "AlarmService", "onDestroy")
        stopSound()
        stopVibration()
        super.onDestroy()
    }
    ...
    private fun startSound() {
        ...
        try {
            // 1) 타이머 전용 사용자 지정 사운드(Downloads에서 선택한 mp3 등) 우선
            //    - 먼저 앱 내부로 복사된 파일이 있으면 그걸 사용(권한 이슈/한글 파일명... 안정성)
            //    - 단, 사용자가 원본 파일을 삭제한 경우에는(열기 실패) 반드시 기본으로 복귀해야 함
            TimerAlarmPrefs.validateOrResetToDefault(this)
            val localPath = TimerAlarmPrefs.getCustomLocalPath(this)
            val customUri = TimerAlarmPrefs.getReadableCustomUriOrNull(this)
            ...
            mediaPlayer = MediaPlayer().apply {
                ...
                if (!localPath.isNullOrBlank() && java.io.File(localPath).exists()) {
                    ...
                    // SAF/MediaStore Uri
                    setDataSource(this@AlarmService, customUri)
                } else {
                    // 2) 기본 내장 사운드(res/raw/alarm_sound.mp3)
            ...
            AppLog.e(applicationContext, "AlarmService", "startSound failed; fallback to default", e)
            // 사용자 파일 문제가 있으면 기본으로 되돌리고 재시도
            TimerAlarmPrefs.clearCustom(this)
            runCatching {
                val afd = resources.openRawResourceFd(R.raw.alarm_sound)
                mediaPlayer = MediaPlayer().apply {
                    setAudioAttributes(attrs)
    ...
    private fun postHistoryNotification(label: String) {
        ...
        // ✅ 중복 알람 기록 방지 (2초 이내 같은 레이블은 무시)
        val prefs = getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val lastAlarmTime = prefs.getLong("last_alarm_time", 0L)
        val lastAlarmLabel = prefs.getString("last_alarm_label", "")
        val now = System.currentTimeMillis()

        // 같은 레이블의 알람이 2초 이내에 다시 울리면 기록 생략
        if (label == lastAlarmLabel && (now - lastAlarmTime) < 2000L) {
            return
        }

        // 마지막 알람 정보 저장
        prefs.edit()
            .putLong("last_alarm_time", now)
            .putString("last_alarm_label", label)
            .apply()

        val time = SimpleDateFormat("HH:mm:ss", Locale.getDefault()).format(Date())
        val text = "$time • \"$label\" 타이머 종료"

        val n = NotificationCompat.Builder(this, HISTORY_CHANNEL_ID)
            .setSmallIcon(R.drawable.ic_notification)
            .setContentTitle("타이머 종료 시각")
            .setContentText(text)
