...
class ClockService : Service() {
                    ...
                    if (hasExtras) {
                        ...
                        runCatching {
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                                startForeground(
                                    FOREGROUND_ID,
                                    buildExtraSummaryForegroundNotification(),
                                ...
                                startForeground(FOREGROUND_ID, buildExtraSummaryForegroundNotification())
                            }
                        }
                    } else {
    ...
    private fun handleMainTimerExpiredFromAlarm(endAtWallMsFromAlarm: Long) {
        ...
        val activeEnd = if (timerEndElapsed > 0L) timerEndElapsed else persistedEnd
        ...
        if (endAtWallMsFromAlarm > 0L) {
            val diff = kotlin.math.abs(activeEnd - endAtWallMsFromAlarm)
            if (diff > 2_000L) {
                // 다른 타이머(구 알람)일 가능성
                android.util.Log.d("ClockService", "⚠️ MAIN_TIMER_ALARM_FIRED 무시: end mismatch active=$activeEnd alarm=$endAtWallMsFromAlarm")
                return
            }
        }
    ...
    private fun ensureForeground(preferred: Leader) {
        if (!isInForeground) {
            ...
            val initial: Notification = when (preferred) {
                ...
                Leader.EXTRA -> buildExtraSummaryForegroundNotification() // 이 함수가 있을 때만
            }
    ...
    private fun onChannelPossiblyIdle() {
        ...
        when {
            ...
            stopwatch2Running -> {
                foregroundLeader = Leader.STOPWATCH2
                ...
                runCatching {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                        startForeground(
                            FOREGROUND_ID,
                            buildStopwatch2Notification(elapsed),
                            ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK
                        )
                    } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        startForeground(
                        ...
                        startForeground(FOREGROUND_ID, buildStopwatch2Notification(elapsed))
                    }
                }
            }
            extraRunning -> {
    ...
    private fun notifyStopwatch2(elapsedMs: Long) {
        val id = if (foregroundLeader == Leader.STOPWATCH2) FOREGROUND_ID else NID_STOPWATCH2
        if (id == FOREGROUND_ID) {
            cancelSafe(NID_STOPWATCH2)
        }
    ...
    private fun scheduleExtraFinishAlarm(id: String, label: String, endAtMs: Long) {
        val am = getSystemService(Context.ALARM_SERVICE) as AlarmManager
        ...
        try {
            ...
            am.set(AlarmManager.RTC_WAKEUP, endAtMs, pi)
        }
    }
