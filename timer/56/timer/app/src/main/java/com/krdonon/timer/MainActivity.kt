...
class MainActivity : AppCompatActivity() {
     ...
     *
     * ⚠️ 중요:
     * - savedInstanceState != null 인 경우(프로세스 복원/구성 변경 등) 프래그먼트의 isVisible 판정이
     *   onCreate 시점에는 불안정할 수 있습니다.
     * - 이때 "보이는 프래그먼트를 못 찾았다"는 이유로 STOPWATCH 를 선택/저장해버리면,
     *   다음 실행 때 기본 탭이 STOPWATCH 로 고정되는 현상이 발생할 수 있습니다.
     *
     * 따라서 여기서는 "UI만" 맞추고, 마지막 탭 저장(SharedPreferences)은 절대 덮어쓰지 않습니다.
     */
    private fun syncSelectedTabFromVisibleFragment() {
        ...
        // "보이는 프래그먼트"를 확실히 찾았을 때만 그 값을 사용하고,
        // 못 찾으면 기존 저장값을 그대로 사용한다(기본은 TIMER).
        val tabForUi = when {
            alarmVisible -> BottomTab.ALARM
            timerVisible -> BottomTab.TIMER
            else -> loadLastTab()
        }
        setSelectedTab(tabForUi, persistLastTab = false)
    }
     ...
     * @param persistLastTab true 이면 SharedPreferences 에 마지막 탭을 저장한다.
     */
