...
class TimerFragment : Fragment() {
    ...
    private fun shouldIgnoreRunningBroadcast(remain: Long, sessionId: Long, endElapsed: Long): Boolean {
        ...
        if (syncState == STATE_FINISHED || syncState == STATE_STOPPED) {
            if (syncSessionId == 0L || syncSessionId == sessionId) {
        ...
        if (now - lastTerminalStateAt < 3000L) {
            AppLog.d(requireContext(), "TimerFragment", "ignore RUNNING because terminal state was received recently remain=$remain")
            return true
        }
        return false
    }
