...
class TimerFragment : Fragment() {
    ...
    private var pendingMainInputMode: String = "duration"
    ...
    private val timerStateReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (state == lastProcessedState && (now - lastProcessedTime) < 100) {
                AppLog.d(requireContext(), "TimerFragment", "skip duplicate broadcast state=$state")
                return
            }
    ...
    private fun processBroadcastState(state: String, remain: Long, endElapsed: Long, sessionId: Long) {
        if (isProcessingBroadcast) {
            AppLog.d(requireContext(), "TimerFragment", "already processing broadcast, requeue state=$state")
            handler.postDelayed({
                processBroadcastState(state, remain, endElapsed, sessionId)
            }, 100)
            return
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        parentFragmentManager.setFragmentResultListener("numberPadResult", viewLifecycleOwner) { _, bundle ->
            ...
            val clockMode = bundle.getBoolean("clockMode", false)
            val isTomorrow = bundle.getBoolean("isTomorrow", false)
            val ampmSelected = bundle.getBoolean("ampmSelected", false)
            val isAm = bundle.getBoolean("isAm", true)
            AppLog.i(
                requireContext(),
                "TimerFragment",
                "numberPadResult targetAt=$targetAt duration=$dur clockMode=$clockMode isTomorrow=$isTomorrow ampmSelected=$ampmSelected isAm=$isAm now=${System.currentTimeMillis()}"
            )
            when {
    ...
    private fun startMainFromRemain(remain: Long, exactEndAt: Long? = null) {
        ...
        val endAt = exactEndAt ?: (nowWall + remain)
        val normalizedRemain = (endAt - nowWall).coerceAtLeast(0L)

        // 표시/계산은 ELAPSED_REALTIME 기반
        mainEndElapsed = SystemClock.elapsedRealtime() + normalizedRemain
        pendingMainTargetAtMs = endAt
        timeLeftInMillis = normalizedRemain
        running = true
        btnStart.text = getString(R.string.btn_pause)
        startMainTicker()
        ...
        AppLog.i(requireContext(), "TimerFragment", "startMainFromRemain remain=$remain normalizedRemain=$normalizedRemain endAt=$endAt inputMode=$pendingMainInputMode")
    }

    private fun startMain() {
        val targetAt = pendingMainTargetAtMs
        val remain = targetAt?.let {
        ...
        timeLeftInMillis = remain
        AppLog.i(requireContext(), "TimerFragment", "startMain remain=$remain pendingTargetAt=$targetAt inputMode=$pendingMainInputMode")
        if (targetAt != null) {
            startMainFromRemain(remain, targetAt)
            runCatching { ClockService.startTimerAt(requireContext(), targetAt) }
        } else {
            startMainFromRemain(remain, null)
    ...
    private fun updateMainTimerUI() {
        ...
        val alarmText = if (timeLeftInMillis <= 0L) {
            ...
            val exactTarget = pendingMainTargetAtMs ?: viewModel.mainEndAtMs.value
            val endAt = when {
    ...
    private fun restoreMainState() {
        ...
        mainTimer = object : CountDownTimer(remain, 50L) {
            ...
            override fun onFinish() {
                // 복원용 CountDownTimer도 UI 보조용일 뿐이다.
                // 여기서 stopTimer()를 호출하면 정확 알람보다 먼저 서비스가 꺼질 수 있다.
                running = false
                timeLeftInMillis = 0L
                btnStart.text = getString(R.string.btn_start)
                timerText.text = formatDuration4(0L)
                nextTimeText.text = getString(R.string.next_time_prefix, "--")
                AppLog.d(requireContext(), "TimerFragment", "restored UI countdown reached zero; waiting for service/alarm finalization")
            }
        }.start()
        ...
        if (state != null) {
            ...
            when (state) {
                ...
                STATE_RUNNING -> {
                    ...
                    val nowElapsed = SystemClock.elapsedRealtime()
                    val restoredEndElapsed = nowElapsed + remain
