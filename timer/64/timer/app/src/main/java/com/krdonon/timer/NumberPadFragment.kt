...
class NumberPadFragment : Fragment() {
    ...
    private fun applyClockModeUiState() {
        if (isTomorrow || ampmSelected) {
            // 시각 지정 모드에서는 초/밀리초를 항상 0으로 고정한다.
            digits[2].fill('0')
            digits[3].fill('0')
            cursors[2] = 0
            cursors[3] = 0
        ...
        btnTomorrow.alpha = if (isTomorrow) 1.0f else 0.75f
        btnAmPm.alpha = if (ampmSelected) 1.0f else 0.75f
        refreshPreview()
        updateLabelColors()
    }
    ...
    override fun onCreateView(
        ...
        btnTomorrow.setOnClickListener {
            ...
            AppLog.i(requireContext(), "NumberPad", "btnTomorrow clicked isTomorrow=$isTomorrow ampmSelected=$ampmSelected isAm=$isAm")
            applyClockModeUiState()
        }
        ...
        btnAmPm.setOnClickListener {
            ...
            AppLog.i(requireContext(), "NumberPad", "btnAmPm clicked isTomorrow=$isTomorrow ampmSelected=$ampmSelected isAm=$isAm")
            applyClockModeUiState()
        }
    ...
    private fun finishAndReturn() {
        ...
        AppLog.i(
            requireContext(),
            "NumberPad",
            "finishAndReturn isTomorrow=$isTomorrow ampmSelected=$ampmSelected isAm=$isAm rawH=$rawH m=$m s=$s ms=$ms clockMode=$clockMode"
        )
        ...
        bundle.putBoolean("clockMode", clockMode)
        bundle.putBoolean("isTomorrow", isTomorrow)
        bundle.putBoolean("ampmSelected", ampmSelected)
